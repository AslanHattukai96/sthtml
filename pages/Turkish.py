...
st.markdown("<h1 style='text-align: center; color: yellow;'>Adığece Öğren</h1>", unsafe_allow_html=True)
st.title("")

# Set target language to Turkish
...
if st.session_state.circassian_word is not None:
    st.write(f"Aşağıdaki kelimeyi Çerkesçeden Türkçeye çevirin: **{st.session_state.circassian_word}**")

    user_translation = st.text_input(f"Türkçe çeviriniz: ").strip()
    
    # Change button label according to the target language
    submit_button_label = "Kontrol Et"  # Change button label to "Kontrol Et"
